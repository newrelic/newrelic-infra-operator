...
function create_cluster() {
    ...
    rm -rf ./newrelic-infra-operator* > /dev/null
    ...
    # Set GOOS and GOARCH explicitly since Dockerfile expects them in binary name
    GOOS=${GOOS:-linux} GOARCH=${GOARCH:-arm64} make compile > /dev/null

    echo "🔄 Building Docker image"
    docker build --tag e2e/newrelic-infra-operator:e2e . --quiet > /dev/null

    echo "🔄 Loading image into cluster"
    minikube image load e2e/newrelic-infra-operator:e2e --profile ${CLUSTER_NAME} > /dev/null

    echo "🔄 Adding Helm repositories"
    helm repo add newrelic-infra-operator https://newrelic.github.io/newrelic-infra-operator > /dev/null
